...
if len(levenshtein_thresholds) > 0:
    ...
    for thres, m_prob in zip(levenshtein_thresholds, m_probability_or_probabilities_lev):
...
#print(f"datediff: {datediff_thresholds}")
    ...
    for metric, thres, m_prob in zip(datediff_thresholds[0], datediff_thresholds[1], m_probability_or_probabilities_datediff):
...
if len(levenshtein_thresholds) > 0:
    ...
    comparison_desc += (
        f"Dates within the following threshold{plural} {lev_desc} vs. "
    )
