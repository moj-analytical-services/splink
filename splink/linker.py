...
class Linker:
    ...
    def analyse_blocking_rule(self, blocking_rule):
        self._initialise_df_concat_with_tf(materialise=True)
        sql = analyse_blocking_rule_sql(self, blocking_rule)
        self.enqueue_sql(sql, "__splink__analyse_blocking_rule")
        res = self.execute_sql_pipeline().as_record_dict()[0]
        res = res["count_of_pairwise_comparisons_generated"]
        return f"Count of pairwise record comparisons generated by rule: {res:,.0f}"
