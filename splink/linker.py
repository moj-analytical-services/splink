...
class Linker:
    ...
    def __init__(
        ...
        Args:
            ...
            settings_dict (dict: A Splink settings dictionary. If not
        ...
        # incompatible dialect not passed. Note `_sql_dialect` is present
        # on dialect specific linkers such as the DuckDBLinker
    ...
    @property
    def _cache_uid(self):
        return self._settings_obj._linker_uuid

    @_cache_uid.setter
    def _cache_uid(self, value):
        self._settings_obj._linker_uuid = value
    ...
    def _sql_to_splink_dataframe_checking_cache(
        ...
        The SQL is often a sequence of CTEs generated by the pipeine
        ...
        # The hash uniquely identifies a particular SQL statement
        #
        # Ambiguity arises because there may be two different tables generated called
        # e.g. `__splink__df_blocked`, that were generated from different blocking rules
        # so it's important we can uniquely identify the table

        # We hash the settings because there is a one-to-one mapping between
        # setting objects and the SQL they generate.
        # This means that to generate a unqiue hash associated with a table
        # it's sufficient to has the settings object
        # i.e. if this hash is the name, this implies that the SQL is the same

        if materialise_as_hash:
            output_tablename = self._tablename_with_hash(
                output_tablename_templated, sql
            )
        else:
            output_tablename = output_tablename_templated

        if use_cache:
            if self._table_exists_in_database(output_tablename):
                logger.debug(
                    f"Using cache for {output_tablename_templated}"
                    f" with physical name {output_tablename}"
                )
                return self._table_to_splink_dataframe(
                    output_tablename_templated, output_tablename
                )
        ...
        splink_dataframe = self._execute_sql_against_backend(
            sql, output_tablename_templated, output_tablename
        )
    ...
    def cluster_pairwise_predictions_at_threshold(
        ...
        self._input_nodes_concat_with_tf(df_predict)
    ...
    def _tablename_with_hash(self, output_tablename_templated, sql=""):
        hash = self._generate_hash_to_uniquely_identify_table(
            output_tablename_templated, sql
        )
        return f"{output_tablename_templated}_{hash}"

    def _generate_hash_to_uniquely_identify_table(
        self, output_tablename_templated: str, sql=""
    ):

        # These tables can be re-used by both the main linker and the training
        # linkers, and are invariant to the settings dictionary
        if (
            output_tablename_templated == "__splink__df_concat_with_tf"
            or output_tablename_templated.startswith("__splink__df_tf_")
        ):
            return self._cache_uid

        # The hash for some tables can be generated from the settings object
        # i.e. for these tables the state of the settings object
        # is enough to guarantee the form of the SQL statement
        if output_tablename_templated in (
            "__splink__df_comparison_vectors",
            "__splink__df_blocked",
            "__splink__df_match_weight_parts",
        ):
            to_hash = json.dumps(self._settings_obj.as_dict()).encode("utf-8")
            hash = hashlib.sha256(to_hash).hexdigest()[:9]
            return hash

        # The SQL for the remaining tables relies on arguments set on the functino
        # call, so we cannot rely on the settings object for the hash
        to_hash = (sql + self._cache_uid).encode("utf-8")
        return hashlib.sha256(to_hash).hexdigest()[:9]

    def register_input_nodes_concat_with_tf(self, input, table_name, overwrite=False):
        tablename_physical = self._tablename_with_hash("__splink__df_concat_with_tf")
        self.register_table(input, tablename_physical, overwrite=overwrite)
