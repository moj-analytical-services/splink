...
def compute_new_parameters(settings_obj: Settings):
    ...
    select {gamma_column} as comparison_vector_value,
           sum(match_probability)/(select sum(match_probability)+0.00000000001 from __splink__df_predict where {gamma_column} != -1) as m_probability,
           sum(1 - match_probability)/(select sum(1 - match_probability)+0.00000000001 from __splink__df_predict where {gamma_column} != -1) as u_probability,
