...
def datediff_sql(col_name_l, col_name_r, date_threshold, date_metric,
                    cast_str=False, date_format='yyyy-MM-dd'):

    if cast_str:
        if date_metric == "day":
            date_f = f"abs(datediff(to_timestamp({col_name_l},'{date_format}'),to_timestamp({col_name_r},'{date_format}')))"
        elif date_metric in ["month", "year"]:
            date_f = f"floor(abs(months_between(to_timestamp({col_name_l},'{date_format}'),to_timestamp({col_name_r}, '{date_format}'))"
            if date_metric == "year":
                date_f += " / 12))"
            else:
                date_f += "))"
