...
def count_comparisons_from_blocking_rule(
    *,
    ...
    Args:
        table_or_tables (SplinkDataFrame or list): Input data as SplinkDataFrame(s).
            Use db_api.register() to create a SplinkDataFrame from raw data.
...
def cumulative_comparisons_to_be_scored_from_blocking_rules_data(
    *,
    ...
    """Calculate cumulative comparisons to be scored from blocking rules.

    Args:
        table_or_tables (SplinkDataFrame or list): Input data as SplinkDataFrame(s).
            Use db_api.register() to create a SplinkDataFrame from raw data.
        blocking_rules: The blocking rules to analyse
        link_type: The link type - "link_only", "dedupe_only" or "link_and_dedupe"
        unique_id_column_name: Defaults to "unique_id".
        max_rows_limit: Maximum rows limit. Defaults to int(1e9).
        source_dataset_column_name: Defaults to None.

    Returns:
        pd.DataFrame: DataFrame containing cumulative comparison counts
    """
    db_api = get_db_api_from_inputs(table_or_tables)
    splink_df_dict = splink_dataframes_to_dict(table_or_tables)
...
def cumulative_comparisons_to_be_scored_from_blocking_rules_chart(
    *,
    ...
    """Generate a chart of cumulative comparisons from blocking rules.
    ...
    Returns:
        ChartReturnType: A Vega-Lite chart
    """
    db_api = get_db_api_from_inputs(table_or_tables)
    splink_df_dict = splink_dataframes_to_dict(table_or_tables)
