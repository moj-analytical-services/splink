    ...
    def altair_chart(self):
        try:
            ...
            raise ModuleNotFoundError(
                "`altair_chart` requires the optional Altair dependency. "
                "Notebook display and basic HTML export do not require Altair.",
                name="altair",
            ) from exc
...
class MatchWeightsChart(SplinkChart[ComparisonLevelDetailedRecord]):
    ...
    def alter_spec_from_data(self, chart_spec):
        ...
        for layer in chart_spec["vconcat"][0]["layer"]:
            if x_encoding := layer["encoding"].get("x"):
                x_encoding["scale"]["domain"] = [-max_value, max_value]
...
class CumulativeBlockingRuleComparisonsGeneratedChart(SplinkChart[ChartRecord]):
    ...
    @property
    def chart_spec(self) -> dict[str, Any]:
        chart_spec = super().chart_spec
        records = list_items_as_dicts(
            cast(Iterable[AsDictable | dict[str, Any]], self.raw_records)
        )
        if records and any(record.get("is_estimate") for record in records):
            sample_proportion = records[0]["record_sample_proportion"]
            sample_percent = sample_proportion * 100
            chart_spec["title"]["text"] = (
                "Estimated Count of Additional Comparisons Generated by Each "
                f"Blocking Rule scaled up from {sample_percent:g}% sample"
            )
        return chart_spec
