...
class LinkerVisualisations:
    ...
    def blocking_rule_performance_chart(
        self,
        df_predict: SplinkDataFrame,
        ...
        The chart splits the comparisons generated by each blocking rule into
        matches and non-matches at an interactive match-weight threshold. Counts
        ...
        Args:
            df_predict (SplinkDataFrame): Output of ``linker.inference.predict()``.
            missing_edges (bool | int, optional): Add a "Missed" row for
                intra-cluster edges that were not generated by any blocking rule.
                ``df_predict`` must contain the scored missing edges with
                ``match_key = -1``; these rows determine how many missed edges exceed
                each match-weight threshold. If ``True``, the total number of missed
                edges is inferred by counting those rows. If a non-negative integer
                is provided, it is used as the total instead. This is useful when
                ``df_predict`` contains only a filtered subset of the missing edges,
                but their full count was computed separately. If ``False``, missing
                edges are not shown. Defaults to ``False``.
        ...
        Examples:
            ...
            linker.visualisations.blocking_rule_performance_chart(df_predict)
            ```

        Returns:
            BlockingRulePerformanceChart: The interactive chart.
        """
        records = blocking_rule_performance_data(
            self._linker,
            df_predict,
            missing_edges,
        )
