...
class LinkerVisualisations:
    ...
    def blocking_rule_performance_chart(
        ...
        Args:
            df_predict (SplinkDataFrame): Output of ``linker.inference.predict()``.
            missing_edges (bool | int, optional): Include prediction rows whose
                ``match_key`` is ``-1`` as intra-cluster edges missed by every
                blocking rule. If ``True``, their total is counted from
