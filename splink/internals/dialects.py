...
class SplinkDialect(ABC):
    ...
    def proportion_sample_sql(
        ...
        Sampling is a pure function of the composite unique id, so the same rows
        are selected on every run.  This matches the deterministic, hash-based
        approach used elsewhere in Splink (e.g. chunking and EM record sampling)
        and avoids any reliance on backend-specific random sampling or scan order.
