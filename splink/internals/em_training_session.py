...
from .expectation_maximisation import (
    can_precompute_tf_adjustments,
    expectation_maximisation,
    precompute_tf_adjustments_sql,
)
...
class EMTrainingSession:
    ...
    def _comparison_vectors(
        self, precompute_tf_adjustments: bool = False
    ) -> SplinkDataFrame:
        ...
        pipeline.enqueue_list_of_sqls(sqls)
        if precompute_tf_adjustments:
            # Materialise only the gamma values and fixed TF weights needed by EM.
            pipeline.enqueue_sql(
                precompute_tf_adjustments_sql(self.core_model_settings.comparisons),
    ...
    def _train(self, cvv: SplinkDataFrame | None = None) -> CoreModelSettings:
        precomputed_tf_adjustments = can_precompute_tf_adjustments(
            self.estimate_without_term_frequencies,
            self.training_fixed_probabilities,
            self.core_model_settings.comparisons,
        )
        df_precomputed_tf = None
        if cvv is None:
            self._ensure_em_sampling_settings()
            cvv = self._comparison_vectors(precomputed_tf_adjustments)
            if precomputed_tf_adjustments:
                df_precomputed_tf = cvv
        ...
        try:
            ...
            if not cvv.as_record_list(limit=1):
                ...
                raise EMTrainingException(
                    ...
                    "the number of comparisons that will be generated "
                    "by a blocking rule."
                )

            if precomputed_tf_adjustments and df_precomputed_tf is None:
                pipeline = CTEPipeline([cvv])
                pipeline.enqueue_sql(
                    precompute_tf_adjustments_sql(self.core_model_settings.comparisons),
                    "__splink__df_comparison_vectors_with_tf",
                )
                df_precomputed_tf = self.db_api.sql_pipeline_to_splink_dataframe(
                    pipeline
                )
                cvv = df_precomputed_tf

            # Compute the new parameters in the copied settings object.
            ...
            if df_precomputed_tf is not None:
                df_precomputed_tf.drop_table_from_database_and_remove_from_cache()
