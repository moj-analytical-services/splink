...
(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ...
    : ((global =
      ...
      factory((global.splink_vis_utils = {})));
  ...
  function inspectExpanded(object, _, name, proto) {
    ...
    if (object instanceof Map) {
      ...
      tag = `Immutable.${n.name}${
        n.name === "Record" ? "" : `(${object.size})`
      }`;
      arrayish = n.arrayish;
      fields = n.arrayish
        ? iterateImArray$1
  ...
  function format(date, fallback) {
    ...
    return `${formatYear$1(
      date.getUTCFullYear()
    )}-${pad$1(date.getUTCMonth() + 1, 2)}-${pad$1(date.getUTCDate(), 2)}${hours || minutes || seconds || milliseconds ? `T${pad$1(hours, 2)}:${pad$1(minutes, 2)}${seconds || milliseconds ? `:${pad$1(seconds, 2)}${milliseconds ? `.${pad$1(milliseconds, 3)}` : ``}` : ``}Z` : ``}`;
  }
    ...
    switch (type) {
      ...
      default: {
        ...
        if (value instanceof Date) {
          (type = "date"), (value = formatDate$2(value));
          break;
        }
        if (value === FORBIDDEN) {
        ...
        switch (toString.call(value)) {
          ...
          default:
            return (
              expand ? inspectExpanded : inspectCollapsed
            )(value, shallow, name, proto);
        }
        break;
      }
    }
  ...
  class AbstractFile {
    ...
    async image(props) {
      ...
      return new Promise((resolve, reject) => {
        const i = new Image();
        if (
          new URL(url, document.baseURI).origin !== new URL(location).origin
        ) {
    ...
    function Library(resolver) {
      ...
      Object.defineProperties(
        ...
        properties({
          ...
          aq: () =>
            require.alias({ "apache-arrow": arrow.resolve() })(
              arquero.resolve()
            ),
  ...
  function variable_define(name, inputs, definition) {
    ...
    return variable_defineImpl.call(
      ...
      inputs == null
        ? []
        : map.call(inputs, this._module._resolve, this._module),
  ...
  function module_copy(copy, map) {
    ...
    for (const [name, source] of this._scope) {
      ...
      if (source._definition === identity$1) {
        // import
        ...
        copy.import(
          sourceInput._name,
          name,
          map.get(sourceModule) ||
            (sourceModule._source
              ? sourceModule._copy(
                  new Module(copy._runtime, copy._builtins),
                  map
                ) // import-with
              : sourceModule)
        );
        ...
        copy.define(
          name,
          source._inputs.map(variable_name),
          source._definition
        );
      }
    }
  ...
  function variable_generate(variable, version, generator) {
    ...
    function compute(onfulfilled) {
      return new Promise((resolve) =>
        resolve(generator.next(currentValue))
      ).then(({ done, value }) => {
  ...
  function maybeColumns(data) {
    ...
    if (data.schema && iterable(data.schema.fields))
      return Array.from(data.schema.fields, (f) => f.name); // apache-arrow
  ...
  function ascending(a, b) {
    return (
      defined(b) - defined(a) || (a < b ? -1 : a > b ? 1 : a >= b ? 0 : NaN)
    );
  }

  function descending(b, a) {
    return (
      defined(a) - defined(b) || (a < b ? -1 : a > b ? 1 : a >= b ? 0 : NaN)
    );
  }
  ...
  function createChooser({
    ...
    return function chooser(
      ...
      if (value === undefined)
        value =
          key !== undefined && data instanceof Map
            ? size > 0
              ? Array.from(key, (k) => data.get(k))
              : data.get(key)
            : undefined;
      unique = !!unique;
      data = arrayify(data);
      ...
      if (unique)
        keys = [...new Map(keys.map((o) => [intern(o[0]), o])).values()];
  ...
  function maybeSelection(data, index, value, multiple, valueof) {
    const values = new Set(
      value === undefined ? [] : multiple ? arrayify(value) : [value]
    );
  ...
  function inputof$1(input, multiple) {
    return input === undefined
      ? new OptionZero(multiple ? [] : null)
      : typeof input.length === "undefined"
      ? new (multiple ? MultipleOptionOne : OptionOne)(input)
      : input;
  }
  ...
  function createRange(
    ...
    if (range) {
      ...
      let tmin = +transform(min),
        tmax = +transform(max);
      if (tmin > tmax) [tmin, tmax] = [tmax, tmin];
      range = html`<input
        type="range"
        min=${isFinite(tmin) ? tmin : null}
        max=${isFinite(tmax) ? tmax : null}
        step=${step === undefined ||
        (transform !== identity && transform !== negate)
          ? "any"
          : step}
        name="range"
        oninput=${onrange}
    ...
    const form = html`<form
      ...
      style=${maybeWidth(width)}
    >
      ${maybeLabel(label, number)}
      <div class="__ns__-input">${number}${range}</div>
    </form>`;
    ...
    if (initialValue === undefined && irange)
      initialValue = irange.valueAsNumber; // (min + max) / 2
  ...
  const select = createChooser({
    render(
      ...
      selected,
      disabled,
      { format, multiple, size, label, width }
    ) {
      ...
      >
        ${index.map(
          (i) =>
            html`<option
              value=${i}
              disabled=${typeof disabled === "function" ? disabled(i) : false}
              selected=${selected(i)}
            >
              ${stringify(format(data[i], i, data))}
            </option>`
        )}
      </select>`;
      const form = html`<form class="__ns__" style=${maybeWidth(width)}>
        ${maybeLabel(label, select)}${select}
      </form>`;
      return [form, select];
    },
    selectedIndexes(input) {
  ...
  function table$1(data, options = {}) {
    ...
    const id = newId();
    const root = html`<form
      class="__ns__ __ns__-table"
      id=${id}
      style=${{
    ...
    const tr = html`<tr>
      <td>
        <input
          ...
          name=${multiple ? null : "radio"}
        />
      </td>
      ${columns.map(() => html`<td></td>`)}
    </tr>`;
    const theadr = html`<tr>
      <th>
        <input type="checkbox" onclick=${reselectAll} disabled=${!multiple} />
      </th>
      ${columns.map(
        (column) =>
          html`<th title=${column} onclick=${(event) => resort(event, column)}>
            <span></span>${header && column in header ? header[column] : column}
          </th>`
      )}
    </tr>`;
...
<style>${columns
      .map((column, i) => {
        ...
        if (rules.length)
          return `#${id} tr>:nth-child(${i + 2}){${rules.join(";")}}`;
      })
      .filter(identity)
      .join("\n")}</style>`);
    ...
    function reselect(event) {
      ...
      if (!multiple) {
        ...
        if (anchor === null)
          anchor = selected.size ? first(selected) : index[0];
    ...
    if (minlengthof(1)) {
      ...
      tbody.append(
        html`<tr>
          ${columns.length ? html`<td></td>` : null}
          <td
            rowspan=${columns.length}
            style="padding-left: var(--length3); font-style: italic;"
          >
            No results.
          </td>
        </tr>`
    ...
    return Object.defineProperty(root, "value", {
      get() {
        if (value === undefined) {
          ...
          if (multiple) {
            value = Array.from(
              required && selected.size === 0 ? index : selected,
              (i) => array[i]
            );
  ...
  function formatof(base = {}, data, columns, locale) {
    ...
    for (const column of columns) {
      ...
      switch (type(data, column)) {
        case "number":
          format[column] = formatLocaleNumber(locale);
          break;
        case "date":
          format[column] = formatDate;
          break;
        default:
          format[column] = formatLocaleAuto(locale);
          break;
      }
    }
    return format;
  ...
  function _1$1(md) {
    return md`
# Splink cluster studio (splink3 version)`;
  }

  function _selected_cluster_id(splink_vis_utils, cluster_unique_ids) {
    return splink_vis_utils.select(cluster_unique_ids, {
  ...
  function _edge_colour_metric(splink_vis_utils, raw_edges_data) {
    const node_size_options =
      splink_vis_utils.detect_edge_colour_metrics(raw_edges_data);
  ...
  function _node_size_metric(splink_vis_utils, raw_nodes_data) {
    const node_size_options =
      splink_vis_utils.detect_node_size_metrics(raw_nodes_data);
    let v = splink_vis_utils.select(node_size_options, {
  ...
  function _node_colour_metric(splink_vis_utils, raw_nodes_data) {
    const node_size_options =
      splink_vis_utils.detect_node_colour_metrics(raw_nodes_data);
  ...
  function _show_edge_comparison_type$1(splink_vis_utils) {
    return splink_vis_utils.checkbox(
      new Map([
  ...
  function _score_threshold_filter(splink_vis_utils) {
    return splink_vis_utils.range([-20, 20], {
  ...
  function _corresponding_probability(
    html,
    splink_vis_utils,
    score_threshold_filter
  ) {
    return html`Your chosen threshold corresponds to a match probability of
    ${splink_vis_utils
      .log2_bayes_factor_to_prob(score_threshold_filter)
      .toPrecision(4)}`;
  }

  function _additional_graph_controls(splink_vis_utils) {
    return splink_vis_utils.checkbox(
  ...
  function _edge_table(
    selected_edge,
    html,
    show_edge_comparison_type,
    splink_vis_utils,
    ss
  ) {
  ...
  function _nothing_selected_message$1(
    no_edge_selected,
    no_node_selected,
    html,
    selected_edge
  ...
  function _force_directed_chart(vegaEmbed, splink_vis_utils, spec) {
    return vegaEmbed(splink_vis_utils.cloneDeep(spec));
  }

  function _node_history_table(
    node_history,
    html,
    show_edge_comparison_type,
    splink_vis_utils,
  ...
  function _refresh$1(Inputs) {
    return Inputs.button("refresh splink_vis_utils javascript lib");
  }

  function _cluster_table(
  ...
  function _comparison_columns_table$1(
    no_edge_selected,
    html,
    show_edge_comparison_type,
    splink_vis_utils,
...
## Outputs`;
  ...
  function _ss$1(splink_vis_utils, splink_settings) {
    return new splink_vis_utils.SplinkSettings(JSON.stringify(splink_settings));
  }

  function _no_edge_selected$1(selected_edge) {
    return typeof selected_edge == "undefined";
  }

  function _no_node_selected(selected_node) {
    return typeof selected_node == "undefined";
  }
...
## Data processing`;
  ...
  function _selected_edge$1(observe_chart_data, force_directed_chart) {
    return observe_chart_data(force_directed_chart, "edge_click");
  }

  function _observe_chart_data$1(Generators) {
    return function observe_chart_data(chart, signal_name) {
  ...
  function _selected_node(observe_chart_data, force_directed_chart) {
    return observe_chart_data(force_directed_chart, "node_click");
  }

  function _selected_cluster_metrics(
    raw_clusters_data,
    svu_options,
    selected_cluster_id
  ) {
  ...
  function _filtered_nodes(
    splink_vis_utils,
    raw_nodes_data,
    svu_options,
    selected_cluster_id
  ) {
    return splink_vis_utils.filter_nodes_with_cluster_id(
      raw_nodes_data,
      svu_options.cluster_colname,
      selected_cluster_id
    );
  ...
  function _control_node_history(selected_node, force_directed_chart, $0) {
    if (typeof force_directed_chart._signals.node_click.value == "undefined") {
      ...
      $0.value.unshift(force_directed_chart._signals.node_click.value);
      $0.value = $0.value;
    }
  }
  ...
  function _37(md) {
    return md`
## Following are global variables embedded in final html so not needed in final version`;
  }

  function _named_clusters() {
    return null;
  }
  ...
  function define$1(runtime, observer) {
    ...
    main
      .variable(observer("viewof selected_cluster_id"))
      .define(
        "viewof selected_cluster_id",
        ["splink_vis_utils", "cluster_unique_ids"],
        _selected_cluster_id
      );
    main
      .variable(observer("selected_cluster_id"))
      .define(
        "selected_cluster_id",
        ["Generators", "viewof selected_cluster_id"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof edge_colour_metric"))
      .define(
        "viewof edge_colour_metric",
        ["splink_vis_utils", "raw_edges_data"],
        _edge_colour_metric
      );
    main
      .variable(observer("edge_colour_metric"))
      .define(
        "edge_colour_metric",
        ["Generators", "viewof edge_colour_metric"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof node_size_metric"))
      .define(
        "viewof node_size_metric",
        ["splink_vis_utils", "raw_nodes_data"],
        _node_size_metric
      );
    main
      .variable(observer("node_size_metric"))
      .define(
        "node_size_metric",
        ["Generators", "viewof node_size_metric"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof node_colour_metric"))
      .define(
        "viewof node_colour_metric",
        ["splink_vis_utils", "raw_nodes_data"],
        _node_colour_metric
      );
    main
      .variable(observer("node_colour_metric"))
      .define(
        "node_colour_metric",
        ["Generators", "viewof node_colour_metric"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof show_edge_comparison_type"))
      .define(
        "viewof show_edge_comparison_type",
        ["splink_vis_utils"],
        _show_edge_comparison_type$1
      );
    main
      .variable(observer("show_edge_comparison_type"))
      .define(
        "show_edge_comparison_type",
        ["Generators", "viewof show_edge_comparison_type"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof show_full_tables"))
      .define(
        "viewof show_full_tables",
        ["raw_clusters_data", "splink_vis_utils"],
        _show_full_tables
      );
    main
      .variable(observer("show_full_tables"))
      .define(
        "show_full_tables",
        ["Generators", "viewof show_full_tables"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof score_threshold_filter"))
      .define(
        "viewof score_threshold_filter",
        ["splink_vis_utils"],
        _score_threshold_filter
      );
    main
      .variable(observer("score_threshold_filter"))
      .define(
        "score_threshold_filter",
        ["Generators", "viewof score_threshold_filter"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("corresponding_probability"))
      .define(
        "corresponding_probability",
        ["html", "splink_vis_utils", "score_threshold_filter"],
        _corresponding_probability
      );
    main
      .variable(observer("viewof additional_graph_controls"))
      .define(
        "viewof additional_graph_controls",
        ["splink_vis_utils"],
        _additional_graph_controls
      );
    main
      .variable(observer("additional_graph_controls"))
      .define(
        "additional_graph_controls",
        ["Generators", "viewof additional_graph_controls"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("edge_table"))
      .define(
        "edge_table",
        [
          "selected_edge",
    ...
    main
      .variable(observer("nothing_selected_message"))
      .define(
        "nothing_selected_message",
        ["no_edge_selected", "no_node_selected", "html", "selected_edge"],
        _nothing_selected_message$1
      );
    main
      .variable(observer("viewof force_directed_chart"))
      .define(
        "viewof force_directed_chart",
        ["vegaEmbed", "splink_vis_utils", "spec"],
        _force_directed_chart
      );
    main
      .variable(observer("force_directed_chart"))
      .define(
        "force_directed_chart",
        ["Generators", "viewof force_directed_chart"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("node_history_table"))
      .define(
        "node_history_table",
        [
          "node_history",
    ...
    main
      .variable(observer("viewof refresh"))
      .define("viewof refresh", ["Inputs"], _refresh$1);
    main
      .variable(observer("refresh"))
      .define("refresh", ["Generators", "viewof refresh"], (G, _) =>
        G.input(_)
      );
    main
      .variable(observer("cluster_table"))
      .define(
        "cluster_table",
        [
          "selected_cluster_metrics",
    ...
    main
      .variable(observer("comparison_columns_table"))
      .define(
        "comparison_columns_table",
        [
          "no_edge_selected",
          "html",
          "show_edge_comparison_type",
        ...
        _comparison_columns_table$1
      );
    main
      .variable(observer("waterfall_chart"))
      .define(
        "waterfall_chart",
        [
          "no_edge_selected",
    ...
    main
      .variable(observer("edges_full_table"))
      .define(
        "edges_full_table",
        ["show_full_tables", "html", "splink_vis_utils", "filtered_edges"],
        _edges_full_table
      );
    main
      .variable(observer("nodes_full_table"))
      .define(
        "nodes_full_table",
        [
          "show_full_tables",
    ...
    main
      .variable(observer("clusters_full_table"))
      .define(
        "clusters_full_table",
        ["show_full_tables", "html", "splink_vis_utils", "raw_clusters_data"],
        _clusters_full_table
      );
    main.variable(observer()).define(["md"], _22$1);
    main
      .variable(observer("spec"))
      .define(
        "spec",
        [
          "splink_vis_utils",
    ...
    main
      .variable(observer("ss"))
      .define("ss", ["splink_vis_utils", "splink_settings"], _ss$1);
    main
      .variable(observer("no_edge_selected"))
      .define("no_edge_selected", ["selected_edge"], _no_edge_selected$1);
    main
      .variable(observer("no_node_selected"))
      .define("no_node_selected", ["selected_node"], _no_node_selected);
    main.variable(observer()).define(["md"], _27);
    main
      .variable(observer("cluster_unique_ids"))
      .define(
        "cluster_unique_ids",
        ["splink_vis_utils", "raw_nodes_data", "svu_options"],
        _cluster_unique_ids
      );
    main
      .variable(observer("selected_edge"))
      .define(
        "selected_edge",
        ["observe_chart_data", "force_directed_chart"],
        _selected_edge$1
      );
    main
      .variable(observer("observe_chart_data"))
      .define("observe_chart_data", ["Generators"], _observe_chart_data$1);
    main
      .variable(observer("selected_node"))
      .define(
        "selected_node",
        ["observe_chart_data", "force_directed_chart"],
        _selected_node
    ...
    main
      .variable(observer("selected_cluster_metrics"))
      .define(
        "selected_cluster_metrics",
        ["raw_clusters_data", "svu_options", "selected_cluster_id"],
        _selected_cluster_metrics
      );
    main
      .variable(observer("filtered_nodes"))
      .define(
        "filtered_nodes",
        [
          "splink_vis_utils",
    ...
    main
      .variable(observer("filtered_edges"))
      .define(
        "filtered_edges",
        [
          "splink_vis_utils",
    ...
    main
      .variable(observer("mutable node_history"))
      .define(
        "mutable node_history",
        ["Mutable", "initial node_history"],
        (M, _) => new M(_)
      );
    main
      .variable(observer("node_history"))
      .define("node_history", ["mutable node_history"], (_) => _.generator);
    main
      .variable(observer("control_node_history"))
      .define(
        "control_node_history",
        ["selected_node", "force_directed_chart", "mutable node_history"],
        _control_node_history
      );
    main.variable(observer()).define(["md"], _37);
    main
      .variable(observer("named_clusters"))
      .define("named_clusters", _named_clusters);
    return main;
  }
...
# splink_comparison_viewer`;
  ...
  function _show_edge_comparison_type(splink_vis_utils) {
    return splink_vis_utils.checkbox(
      new Map([
  ...
  function _gamma_filters(get_gammas_filters, ss) {
    return get_gammas_filters(ss);
  }

  function _filter_count(splink_vis_utils) {
  ...
  function _comparison_vector_distribution_chart(
    vegaEmbed,
    chart_spec_with_data
  ) {
    return vegaEmbed(chart_spec_with_data);
  }

  function _nothing_selected_message(no_edge_selected, html) {
    if (no_edge_selected) {
      return html`<span style="color:red; font-weight:bold; background-color:
      #FFFF00"'>Click on a bar in the comparison vector chart above to show an
      example record comparison`;
    }

    return html``;
  }

  function _example_index(
    no_edge_selected,
    comparison_vector_row_lookup,
    cv_chart_selection,
    splink_vis_utils
  ) {
    if (!no_edge_selected) {
  ...
  function _compairson_non_null_table(
    no_edge_selected,
    html,
    show_edge_comparison_type,
    splink_vis_utils,
    ...
    if (show_edge_comparison_type.includes("show_simple_comparison_table")) {
      return html`
        <h3>Comparison of non-null fields</h3>
        ${splink_vis_utils.edge_row_to_table(selected_edge, ss)}
      `;
    }
  ...
  function _comparison_columns_table(
    no_edge_selected,
    html,
    show_edge_comparison_type,
    splink_vis_utils,
    ...
    if (
      ...
      return html` <h3>Record comparison and associated case expression</h3>
  ...
  function _refresh(Inputs) {
    return Inputs.button("refresh splink_vis_utils javascript lib");
  }

  function _get_gammas_filters(html, splink_vis_utils) {
...
## Interations`;
  ...
  function _cv_chart_selection(
    observe_chart_data,
    comparison_vector_distribution_chart
  ) {
    return observe_chart_data(
      comparison_vector_distribution_chart,
      "gam_concat_signal"
    );
  }

  function _selected_edge(
    no_edge_selected,
    comparison_vector_row_lookup,
    cv_chart_selection,
    example_index
  ...
  function _no_edge_selected(cv_chart_selection) {
    return !("gam_concat" in cv_chart_selection);
  }
...
## Other`;
  ...
  function _ss(splink_vis_utils, splink_settings) {
    return new splink_vis_utils.SplinkSettings(JSON.stringify(splink_settings));
  }

  function _filtered_comparison_vector_data(
    gamma_filters,
...
## Functions`;
  ...
  function _create_comparison_vector_row_lookup() {
    return function create_comparison_vector_row_lookup(sample_edges) {
      let lookup = {};
  ...
  function _comparison_vector_row_lookup(
    create_comparison_vector_row_lookup,
    comparison_vector_data
  ) {
    return create_comparison_vector_row_lookup(comparison_vector_data);
  }

  function _observe_chart_data(Generators) {
    return function observe_chart_data(chart, signal_name) {
      return Generators.observe(function (notify) {
        const signaled = (name, value) => notify(chart.signal(signal_name));
        chart.addSignalListener(signal_name, signaled);
...
## External libs`;
  ...
  function _chart_spec_with_data(
    splink_vis_utils,
...
## Data`;
  ...
  function define(runtime, observer) {
    ...
    main
      .variable(observer("viewof sort_bars_option"))
      .define(
        "viewof sort_bars_option",
        ["splink_vis_utils"],
        _sort_bars_option
      );
    main
      .variable(observer("sort_bars_option"))
      .define(
        "sort_bars_option",
        ["Generators", "viewof sort_bars_option"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("viewof gamma_filters"))
      .define(
        "viewof gamma_filters",
        ["get_gammas_filters", "ss"],
        _gamma_filters
      );
    main
      .variable(observer("gamma_filters"))
      .define("gamma_filters", ["Generators", "viewof gamma_filters"], (G, _) =>
        G.input(_)
      );
    main
      .variable(observer("viewof filter_count"))
      .define("viewof filter_count", ["splink_vis_utils"], _filter_count);
    main
    ...
    main
      .variable(observer("viewof comparison_vector_distribution_chart"))
      .define(
        "viewof comparison_vector_distribution_chart",
        ["vegaEmbed", "chart_spec_with_data"],
        _comparison_vector_distribution_chart
      );
    main
      .variable(observer("comparison_vector_distribution_chart"))
      .define(
        "comparison_vector_distribution_chart",
        ["Generators", "viewof comparison_vector_distribution_chart"],
        (G, _) => G.input(_)
      );
    main
      .variable(observer("nothing_selected_message"))
      .define(
        "nothing_selected_message",
        ["no_edge_selected", "html"],
        _nothing_selected_message
      );
    main
      .variable(observer("viewof example_index"))
      .define(
        "viewof example_index",
        [
          "no_edge_selected",
          "comparison_vector_row_lookup",
          "cv_chart_selection",
    ...
    main
      .variable(observer("example_index"))
      .define("example_index", ["Generators", "viewof example_index"], (G, _) =>
        G.input(_)
      );
    main
      .variable(observer("compairson_non_null_table"))
      .define(
        "compairson_non_null_table",
        [
          "no_edge_selected",
          "html",
          "show_edge_comparison_type",
    ...
    main
      .variable(observer("viewof refresh"))
      .define("viewof refresh", ["Inputs"], _refresh);
    main
      .variable(observer("refresh"))
      .define("refresh", ["Generators", "viewof refresh"], (G, _) =>
        G.input(_)
    ...
    main
      .variable(observer("cv_chart_selection"))
      .define(
        "cv_chart_selection",
        ["observe_chart_data", "comparison_vector_distribution_chart"],
        _cv_chart_selection
      );
    main
      .variable(observer("selected_edge"))
    ...
    main
      .variable(observer("no_edge_selected"))
      .define("no_edge_selected", ["cv_chart_selection"], _no_edge_selected);
    main.variable(observer()).define(["md"], _18);
    main
      .variable(observer("new_width"))
      .define("new_width", ["width"], _new_width);
    main
      .variable(observer("ss"))
      .define("ss", ["splink_vis_utils", "splink_settings"], _ss);
    main
      .variable(observer("filtered_comparison_vector_data"))
      .define(
        "filtered_comparison_vector_data",
        ["gamma_filters", "comparison_vector_data", "filter_count"],
        _filtered_comparison_vector_data
      );
    main.variable(observer()).define(["md"], _22);
    main
      .variable(observer("create_comparison_vector_row_lookup"))
      .define(
        "create_comparison_vector_row_lookup",
        _create_comparison_vector_row_lookup
      );
    main
      .variable(observer("comparison_vector_row_lookup"))
      .define(
        "comparison_vector_row_lookup",
        ["create_comparison_vector_row_lookup", "comparison_vector_data"],
        _comparison_vector_row_lookup
      );
    main
      .variable(observer("observe_chart_data"))
      .define("observe_chart_data", ["Generators"], _observe_chart_data);
    main.variable(observer()).define(["md"], _26);
    main
      .variable(observer("chart_spec_with_data"))
      .define(
        "chart_spec_with_data",
        [
          "splink_vis_utils",
  ...
  var marks = [
      ...
      transform: [
        {
          ...
          forces: [
            ...
            {
              description:
                "Uses link-data to find links between nodes constraining x and y of nodes.  Tranforms link-data so source and target are objects that include e.g. x and y coords",
              force: "link",
              links: "link-data",
              distance: {
    ...
    {
      description:
        "The force link transform will replace source and target with objects containing x and y coords.  We need to extract x and y to plot a path between them",
      type: "path",
      name: "edges",
      from: {
        data: "link-data",
  ...
  class ForceDirectedChart {
    ...
    set_starting_width(new_width) {
      let width_signal = find_obj_in_list(
        this.spec.signals,
        "name",
        "vis_width"
      );
  ...
  function format_nodes_data_for_force_directed(nodes_data, splink_settings) {
  ...
  function format_edges_data_for_force_directed(edge_data, splink_settings) {
    if (splink_settings.settings_dict.link_type == "dedupe_only") {
      let c = splink_settings.settings_dict.unique_id_column_name;
  ...
  function get_unique_cluster_ids_from_nodes_data(nodes_data, cluster_field) {
        ...
        },
        {
          ...
          encoding: {
            ...
            x: {
              ...
              axis: {
                labelExpr:
                  "datum.value == 'Prior lambda' || datum.value == 'Final score' ? '' : datum.value",
                labelAngle: -20,
                labelAlign: "center",
                labelPadding: 10,
                title: "Column",
  ...
  var vconcat = [
    {
      ...
      title: {
        subtitle:
          "Click bars on upmost chart to display example. Brush bottom chart to zoom. ",
    ...
    },
    {
      encoding: {
        ...
        x: {
          ...
          title:
            "Comparison vector value.  (Brush bottom chart to zoom. Click to reset.)",
          type: "nominal",
          axis: {
            labels: false,
            ticks: 0,
