...
def solve_connected_components(
    ...
    while root_rows > 0:
        ...
        for batch in range(batching):
            ...
            # if batch == 0, use materialised df, else add to pipeline
            if batch > 0:
                representatives_table = f"repr_{batch-1}"
            ...
            linker.enqueue_sql(sql, f"r_{batch}")
            # Update our rep_match column in the representatives table.
            sql = _cc_update_representatives_loop_cond(
                f"r_{batch}", representatives_table
            )
            linker.enqueue_sql(sql, f"repr_{batch}")

        # Execute our batched SQL
        representatives = linker.execute_sql_pipeline()
