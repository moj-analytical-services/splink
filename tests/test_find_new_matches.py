...
def test_tf_tables_init_works(test_helpers, dialect, fake_1000):
...
@mark_with_dialects_excluding()
def test_matches_work(test_helpers, dialect, fake_1000, tmp_path):
    helper = test_helpers[dialect]

    linker = helper.linker_with_registration(fake_1000, get_settings_dict())
    ...
    linker.visualisations.match_weights_chart().save(f"{tmp_path}/mwc.html")
