...
def test_u_train_link_only():
    ...
    """
    self_table_count = linker._sql_to_splink_dataframe_checking_cache(check_blocking_sql, "__splink__df_blocked_same_table_count")
...
def test_u_train_link_only_sample():
    ...
    df_l = pd.DataFrame(np.random.randint(0, 1000, size=(1000, 1)), columns=["name"]).reset_index().rename(columns={"index": "unique_id"})
    df_r = pd.DataFrame(np.random.randint(0, 1000, size=(1000, 1)), columns=["name"]).reset_index().rename(columns={"index": "unique_id"})
