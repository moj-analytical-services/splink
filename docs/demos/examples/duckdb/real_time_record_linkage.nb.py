...
#
# It's now possible to compute a match weight for any two records using `linker.inference.score_pairs()`
#
...
# To `score_pairs` the linker needs to compute term frequency tables
...
df_two = linker.inference.score_pairs(record_1, record_2)
df_two.as_pandas_dataframe()

# %% [markdown]
# ### Step 3: Interactive comparisons
#
# One interesting application of `score_pairs` is to create a simple interface that allows the user to input two records interactively, and get real time feedback.
#
...
def myfn(**kwargs):
    ...
    df_two = linker.inference.score_pairs(record_left, record_right)
