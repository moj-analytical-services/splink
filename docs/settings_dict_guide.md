...
- When `link_and_dedupe`, `splink` finds links within and between input datasets.  If  single dataset is provided, it will be deduped. - When `link_only`,  `splink` finds links between datasets, but does not attempt to deduplicate the datasets (it does not try and find links within each input dataset.)
...
The probability that two records chosen at random (with no blocking) are a match.  For example, if there are a million input records and each has on average 1 match, then this value should be 1/1,000,000.

This provides the initial value (prior) from which the EM algorithm will start iterating
...
The maximum number of iterations to run even if convergence has not been reached

Set this value to zero if you do not want to use the EM algorithm and just want to score matches from values you have manually specified in the `m_probabilities` and `u_probabilities` arrays
...
The name of the column in the input dataset representing a unique id

For linking tasks, ids must be unique to each dataset being linked, and do not need to be globally unique across these two datasets
...
A list specifying how records should be compared for probabalistic matching

Comparisons between the values in these columns will be used to determine match scores


### blocking_rules_to_generate_predictions

A list of one or more blocking rules to apply. A cartesian join is applied if `blocking_rules` is empty or not supplied.
...
The name used to refer to this column in the output dataset.  Most useful to describe comparison columns that use multiple input columns.  e.g. a location column that uses postcode and town may be named location
...
An optional label to describe this comparison.
...
A label for this comparson level, which will appear on charts as a reminder of what the level represts
