...
!!! tip "Controlling runtime with `max_pairs`"
    As an alternative (or in addition) to tightening your blocking rule, you can pass `max_pairs` to `estimate_parameters_using_expectation_maximisation` to limit the approximate number of blocked pairwise comparisons used in training. This is a convenient way to control how long a training session takes without having to change your blocking rule:
    ...
    Splink runs a preliminary record-sampled blocking pass to estimate the full blocked-pair count, then applies a deterministic hash-based filter to the input records so that the resulting number of training pairs is approximately `max_pairs`.
...
There are some tools within Splink to help choosing these rules. For example, the `count_comparisons_from_blocking_rules` gives the number of records pairs generated by a blocking rule:
