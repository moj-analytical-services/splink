...
Bias detection, which needs to come before assessing its impact, should be a key part of understanding any linkage pipeline. This means using a bias hypothesis to shape how the model is interpreted. Therefore, this blog will explain an approach for detecting bias in an existing data linking pipeline which has a pre-trained model. There is also a [supplementary notebook]() which demonstrates how this process can work practically using Splink. 

The approach is broken down into 5 key steps:

![Image 1](./img/bias_investigation_steps.png)

These steps will be explained to show how this bias detection process can be used with any data linkage pipeline...

## <u>1. Generate synthetic records</u>

This process builds a neutral foundation for bias detection by creating specific records to evaluate the pre-trained model's results. While the process of generating these records can vary, it's crucial to keep it controlled. Automatically generating large amounts of data without understanding the content can lead to issues similar to using real production data.
